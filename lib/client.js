...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function SettingsPage() {
      const [form, setForm] = React.useState({ appId: '', accessToken: '', resourceId: 'volc.seedasr.sauc.duration', language: 'zh-CN', customHotwords: '', hotkey: settings.hotkey, maxDurationSec: settings.maxDurationSec });
      ...
      const save = async () => {
        ...
        try {
          ...
          const data = await res.json().catch(() => ({}));
          if (res.ok && data?.ok) {
            saveStoredSettings({ hotkey: form.hotkey, maxDurationSec: form.maxDurationSec, lang: form.language });
    ...
    function apply(ctx) {
      ...
      const makeLiveWriter = (sessionId) => {
        let base = null; // 录音开始时的草稿（含用户已输入内容）
        let active = false;
        let lastWritten = null; // 我们上次写入的完整草稿（用于检测用户已发送/清空/改写）
        const writer = {
          begin: () => {
          ...
          set: (text) => {
            ...
            // 发送/清空/替换检测：草稿已不再是我们写入的内容（且不是状态回读延迟）
            if (lastWritten !== null) {
              if (snap.draft === '') return; // 已发送或清空 → 不再写回
              if (snap.draft !== lastWritten && !snap.draft.startsWith(base)) return; // 被整体替换
            }
            const text2 = typeof text === 'string' ? text : '';
            const needSep = base.length > 0 && text2.length > 0
              && !/\s$/.test(base) && !/^\s/.test(text2);
